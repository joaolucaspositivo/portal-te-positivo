...
import {
  deleteTipoSolicitacaoAdmin,
  listTiposSolicitacaoAdmin,
  saveTipoSolicitacaoAdmin,
...
function List() {
  ...
  const listTiposFn = useServerFn(listTiposSolicitacaoAdmin);
  const saveTipoFn = useServerFn(saveTipoSolicitacaoAdmin);
  const deleteTipoFn = useServerFn(deleteTipoSolicitacaoAdmin);
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["public-tipos"] });
    },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Crie e edite os formulários disponíveis ao público.
          </p>
        </div>

        <button
          onClick={() => setEdit({ ativo: true, permite_anonimo: true, ordem: 0 })}
            ...
            {isLoading ? (
              ...
              data.map((t: any) => (
                <tr key={t.id} className="border-t hover:bg-muted/30">
                  <td className="p-3">
                    <Link
                      to="/area-te/tipos-solicitacao/$id"
                      params={{ id: t.id }}
      ...
      {edit && (
        <AdminFormShell
          title={edit.id ? "Editar tipo" : "Novo tipo"}
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="flex items-center gap-2 text-sm col-span-2">
              <input
                type="checkbox"
                checked={!!edit.permite_anonimo}
                onChange={(e) => setEdit({ ...edit, permite_anonimo: e.target.checked })}
              />
