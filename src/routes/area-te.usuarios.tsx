...
function AdminUsers() {
  ...
  const filtered = useMemo(
    () =>
      data.filter((u: any) => {
        ...
        if (q) {
          const text = `${u.email} ${u.profile?.nome_completo ?? ""} ${u.profile?.cargo ?? ""}`.toLowerCase();
  ...
  const pendentes = data.filter((u: any) => (u.profile?.status ?? "pendente") === "pendente").length;

  return (
    <div>
      <h1 className="text-3xl font-bold">Usuários</h1>
      <p className="text-muted-foreground mb-6">
        {data.length} cadastrados {pendentes > 0 && <span className="text-amber-700">· {pendentes} aguardando aprovação</span>}
      </p>
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_180px_180px] mb-4">
        ...
        <select value={statusF} onChange={(e) => setStatusF(e.target.value)} className="px-3 py-2 rounded-md border bg-background text-sm">
          <option value="">Todos status</option>
          {Object.entries(STATUS_LABEL).map(([k, v]) => (
            <option key={k} value={k}>{v}</option>
          ))}
        </select>
        <select value={roleF} onChange={(e) => setRoleF(e.target.value)} className="px-3 py-2 rounded-md border bg-background text-sm">
                  ...
                  <tr key={u.id} className="border-t hover:bg-muted/30">
                    <td className="p-3">
                      <div className="flex items-center gap-3">
                        <UserAvatar path={u.profile?.avatar_url} name={u.profile?.nome_completo ?? u.email} size={36} />
                        <div>
                    ...
                    <td className="p-3 text-muted-foreground">{u.profile?.unidade ?? "—"}</td>
                    <td className="p-3">
                      <div className="flex flex-wrap gap-1">
                        {u.roles.length === 0 ? (
                          ...
                          u.roles.map((r: string) => (
                            <span key={r} className="px-2 py-0.5 rounded text-xs bg-muted">{ROLE_LABEL[r] ?? r}</span>
                          ))
                        )}
                      </div>
                    </td>
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLOR[status]}`}>{STATUS_LABEL[status]}</span>
                    </td>
                    <td className="p-3 text-right">
                      {status === "pendente" && (
                        <button
                          onClick={() => setStatusMut.mutate({ data: { userId: u.id, status: "ativo" } })}
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        <Field label="Nome completo" full>
          <input value={profile.nome_completo} onChange={(e) => setProfile({ ...profile, nome_completo: e.target.value })} className={inpCls} />
        </Field>
        <Field label="Cargo">
          <input value={profile.cargo} onChange={(e) => setProfile({ ...profile, cargo: e.target.value })} className={inpCls} />
        </Field>
        <Field label="Unidade">
          <input value={profile.unidade} onChange={(e) => setProfile({ ...profile, unidade: e.target.value })} className={inpCls} />
        </Field>
        <Field label="Telefone" full>
          <input value={profile.telefone} onChange={(e) => setProfile({ ...profile, telefone: e.target.value })} className={inpCls} />
        </Field>
        <Field label="Bio" full>
          <textarea rows={3} value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })} className={inpCls} />
        </Field>
        ...
        <div className="col-span-2 border-t pt-3 mt-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            {ALL_ROLES.map((r) => (
              <label key={r} className="flex items-center gap-2 text-sm px-3 py-2 rounded border bg-background">
        ...
        <div className="col-span-2 border-t pt-3 mt-1 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {status !== "ativo" && (
              <button type="button"
                onClick={async () => {
                  await setStatusFn({ data: { userId: user.id, status: "ativo" } });
                  toast.success("Aprovado.");
                  qc.invalidateQueries({ queryKey: ["admin-users"] });
                }}
                className="inline-flex items-center gap-1 px-3 py-1.5 text-sm rounded bg-emerald-600 text-white"
              ><UserCheck className="h-4 w-4" /> Aprovar</button>
            )}
            {status !== "bloqueado" && (
              <button type="button"
                onClick={async () => {
                  await setStatusFn({ data: { userId: user.id, status: "bloqueado" } });
                  toast.success("Bloqueado.");
                  qc.invalidateQueries({ queryKey: ["admin-users"] });
                }}
                className="inline-flex items-center gap-1 px-3 py-1.5 text-sm rounded bg-red-600 text-white"
              ><UserX className="h-4 w-4" /> Bloquear</button>
            )}
            <button type="button"
              onClick={async () => {
                try { await resetFn({ data: { email: user.email } }); toast.success("Link de redefinição enviado."); }
                catch (e: any) { toast.error(e.message ?? "Erro."); }
              }}
              className="inline-flex items-center gap-1 px-3 py-1.5 text-sm rounded border"
            ><KeyRound className="h-4 w-4" /> Enviar redefinição de senha</button>
            <button type="button"
              onClick={async () => {
                if (!confirm(`Excluir definitivamente ${user.email}?`)) return;
                try { await deleteFn({ data: { userId: user.id } }); toast.success("Excluído."); qc.invalidateQueries({ queryKey: ["admin-users"] }); onClose(); }
                catch (e: any) { toast.error(e.message ?? "Erro."); }
              }}
              className="inline-flex items-center gap-1 px-3 py-1.5 text-sm rounded border border-destructive text-destructive ml-auto"
            ><Trash2 className="h-4 w-4" /> Excluir conta</button>
          </div>
        </div>
      </div>
    </AdminFormShell>
