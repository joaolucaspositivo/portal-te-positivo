...
function AdminFerramentas() {
  ...
  const listFerramentasFn = useServerFn(listFerramentasAdmin);
  const saveFerramentaFn = useServerFn(saveFerramentaAdmin);
  const deleteFerramentaFn = useServerFn(deleteFerramentaAdmin);

  const { data = [] } = useQuery({
    queryKey: ["admin-ferramentas"],
    queryFn: () => listFerramentasFn(),
  });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">{data.length} cadastradas</p>
        </div>
      ...
      {edit && (
        <AdminFormShell
          title={edit.id ? "Editar ferramenta" : "Nova ferramenta"}
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Status">
              <select
                value={edit.status ?? "Ativa"}
                onChange={(e) => setEdit({ ...edit, status: e.target.value })}
                className={inpCls}
              >
                {STATUS_FERRAMENTA.map((c) => (
                  <option key={c}>{c}</option>
                ))}
              </select>
            </Field>
            ...
            <Field label="Público-alvo">
              <input
                value={edit.publico_alvo ?? ""}
                onChange={(e) => setEdit({ ...edit, publico_alvo: e.target.value })}
                className={inpCls}
              />
            </Field>
            ...
            <Field label="Segmento">
              <input
                value={edit.segmento ?? ""}
                onChange={(e) => setEdit({ ...edit, segmento: e.target.value })}
                className={inpCls}
              />
            </Field>
            ...
            <Field label="Link de acesso" full>
              <input
                value={edit.link_acesso ?? ""}
                onChange={(e) => setEdit({ ...edit, link_acesso: e.target.value })}
                className={inpCls}
              />
            </Field>
            ...
            <Field label="Responsável" full>
              <input
                ...
                onChange={(e) => setEdit({ ...edit, responsavel: e.target.value })}
                className={inpCls}
              />
            </Field>
